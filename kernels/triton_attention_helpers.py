...
def softmax_step(S, M, L, USE_EXP2: tl.constexpr = False):
    ...
    # NX (vLLM PR #56148): keep an empty row's maximum at -inf for later tiles; use a finite shift only for the
    # exponent arithmetic. The old code clamped m_j to 0.0, which under causal masking within a verify block makes a
    # fully-masked (query-row, KV-segment) pair look like a real segment with max=0.0 and corrupts the reduction.
    # Only bites at query_len > 1, which is why it was invisible before the verify path existed.
    safe_m_j = tl.where(m_j > float("-inf"), m_j, 0.0)
    # P : (BLOCK_M, TILE_SIZE)
    # NX: exp -> exp2, gated. On gfx942 `tl.exp` lowers to v_exp_f32 + v_cndmask_b32 + v_ldexp_f32 (a denormal
    # range fixup LLVM inserts because Triton's ExpOpConversionApprox emits llvm.exp2.f32, which preserves denorms).
    # Measured in our own binary: 18 v_exp_f32 alongside 18 v_ldexp_f32, 18 v_cmp_gt_f32 and 36 log2e constants --
    # 6 VALU ops per exponential. tl.exp2 takes Exp2OpConversion -> llvm.amdgcn.exp2.f32 -> bare v_exp_f32, and the
    # dropped denormal handling is irrelevant since P = exp(S - rowmax) <= 1 by construction.
    #
    # GATED because the effect has opposite signs on the two paths, measured over 3 alternating A/B repeats:
    #   prefill (2D path): TTFT -1.47% at 8K, -5.31% at 32K   <- exponentials grow as S^2, so the saving scales
    #   decode  (3D path): TPOT +0.10% at 8K, +1.67% at 32K   <- deterministic (+1.59/+1.59/+1.82), not noise
    # Register pressure, LDS and spills are identical between variants, so the decode cost is not occupancy; most
    # likely instruction scheduling. Unresolved, hence gate rather than apply globally.
    if USE_EXP2:
        P = tl.exp2((S - safe_m_j[:, None]) * 1.4426950408889634)
    else:
    ...
    # alpha : (BLOCK_M, )
    if USE_EXP2:
        alpha = tl.exp2((M - safe_m_j) * 1.4426950408889634)
    else:
