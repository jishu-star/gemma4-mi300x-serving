    ...
    *,
    ...
    if naive_block_assignment:
        return (
            None,
            topk_ids.view(-1),
            # NX: this allocated and filled a ONE-ELEMENT int32 tensor on device
            # every MoE call. It shows up as at::native::FillFunctor<int> --
            # measured at 2.90% of decode kernel time and ~4.5 KiB at 2 GB/s per
            # dispatch, i.e. pure launch latency to write a single integer.
            # Cached per (device, value); the value depends only on topk_ids.numel()
            # and BLOCK_SIZE_M, both of which repeat across steps.
            _cached_num_tokens_post_pad(
                topk_ids.numel() * config["BLOCK_SIZE_M"], topk_ids.device
            ),
        )
...
_NUM_TOKENS_POST_PAD_CACHE: dict[tuple, torch.Tensor] = {}


def _cached_num_tokens_post_pad(value: int, device: torch.device) -> torch.Tensor:
    """Cache the 1-element num_tokens_post_padded tensor.

    The naive-block path rebuilt this every call, emitting a FillFunctor<int>
    dispatch to write one int32. The value is a pure function of
    topk_ids.numel() * BLOCK_SIZE_M, so it repeats across steps at fixed
    batch/tile shape.
    """
    key = (value, device)
    t = _NUM_TOKENS_POST_PAD_CACHE.get(key)
    if t is None:
        t = torch.full((1,), value, dtype=torch.int32, device=device)
        _NUM_TOKENS_POST_PAD_CACHE[key] = t
    return t
