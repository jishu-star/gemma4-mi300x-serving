...
# NX knobs are resolved ONCE at import, not per call.
#
# WHY. sharegpt_c128 is host-dispatch-bound (PROJECT_RECORD: "c128 gap is host dispatch"), so Python
# executed per attention call is charged directly against that cell. The gated prefill arm measured
# c128 -1.12% against a 0.28% ship-vs-ship floor -- a real, reproducible regression -- even though
# its 4096-token context gate makes it IMPOSSIBLE for ShareGPT (bounded at prompt+output <= 2048) to
# reach the modified code path. The cost was evaluating the gate, not taking it: four
# os.environ.get + int() per unified_attention call, measured at 1.548 us per call on this box.
# Module-level constants leave only integer comparisons in the hot path.
#
# CONSEQUENCE FOR PROBES: bm_probe*.py sweep BLOCK_M by rewriting os.environ between calls in one
# process, which works only while the wrapper re-reads it per call. That no longer holds here, so
# probes must keep using blockm/ or pfgate/'s copy; this file is for e2e arms only.
_NX_PF_BM_512 = int(_nx_os.environ.get("NX_PF_BM_512", "0"))
_NX_PF_BM_512_MINSEQ = int(_nx_os.environ.get("NX_PF_BM_512_MINSEQ", "4096"))
_NX_PF_WARPS_512 = int(_nx_os.environ.get("NX_PF_WARPS_512", "0"))
_NX_PF_STAGES_512 = int(_nx_os.environ.get("NX_PF_STAGES_512", "2"))
_NX_PF_TILE_512 = int(_nx_os.environ.get("NX_PF_TILE_512", "0"))
_NX_PF_TILE_512_MINSEQ = int(_nx_os.environ.get("NX_PF_TILE_512_MINSEQ", "0"))
_NX_ATTN_BM256 = int(_nx_os.environ.get("NX_ATTN_BM256", "0"))
_NX_ATTN_BM256_MINSEQ = int(_nx_os.environ.get("NX_ATTN_BM256_MINSEQ", "4096"))
_NX_ATTN_WARPS_256 = int(_nx_os.environ.get("NX_ATTN_WARPS_256", "0"))
_NX_ATTN_STAGES_256 = int(_nx_os.environ.get("NX_ATTN_STAGES_256", "2"))
import os
import torch
...
# NX (vLLM PR #56148): the verify step of speculative decoding has query_len = 1 + num_spec > 1, which the 3D split-KV
# path used to reject outright -- leaving attention at very low occupancy exactly when it is on the critical path.
# Gating it on a PROVEN-uniform decode batch (all query lens equal, no prefill rows) and a bounded query length makes
# the 3D launch safe there. NX_VERIFY_SPLIT lets us A/B it; upstream has it always on.
MAX_UNIFORM_DECODE_QUERY_LEN = 5
NX_VERIFY_SPLIT = os.environ.get("NX_VERIFY_SPLIT", "0") == "1"
...
def kernel_unified_attention(
    ...
    for j in range(loop_lo, loop_hi):
        ...
        # NX: exp2 ON THE PREFILL PATH ONLY. tl.exp lowers on gfx942 to v_exp_f32 + v_cndmask_b32 +
        # v_ldexp_f32 (a denormal range fixup LLVM inserts); tl.exp2 lowers to a bare v_exp_f32, and
        # the dropped denormal handling is irrelevant because P = exp(S - rowmax) <= 1 by construction.
        # The effect has OPPOSITE SIGNS on the two paths, measured over 3 alternating A/B repeats:
        #   prefill (2-D): TTFT -1.47% @8K, -5.31% @32K   <- exponentials grow as S^2, so it scales
        #   decode  (3-D): TPOT +0.10% @8K, +1.67% @32K   <- deterministic, not noise
        # so it must be gated, not applied globally. IS_3D is exactly the decode/verify path, so
        # `not IS_3D` selects prefill.
        #
        # WHY THIS IS BEING RE-DONE: the change was measured once on output_tok_s across all 7 cells,
        # came back inside every noise floor, and was filed inconclusive. But it is prefill-gated, so
        # output_tok_s structurally CANNOT see it -- the right metric is TTFT. It never shipped.
        # At long32k_c1 prefill is 72% of the cell (TTFT 2175 ms of 3017 ms total), which is also
        # where our largest remaining gap to an H100 sits (their TTFT 965 ms, 2.25x faster).
        M, L, P, alpha = softmax_step(S, M, L, USE_EXP2=(not IS_3D))
        acc = acc * alpha[:, None]
...
def reduce_segments(
    ...
    # Empty segments must not set the maximum. Unvisited segments retain the M=-inf, L=1 initial state.
...
def unified_attention(
    ...
    # NX_PF_BM_512 / NX_PF_WARPS_512: BLOCK_M for the head-512 layers on the PREFILL path.
    #
    # THE IDENTITY THAT MATTERS. For this kernel the arithmetic intensity of the K/V stream is
    #     FLOP/byte = (BLOCK_M * TILE * d * 2 * 2) / (TILE * d * 2 * 2) = BLOCK_M
    # TILE cancels exactly. That is why the TILE sweep could not work: TILE=32 and TILE=16 both move
    # ~550 GB per 32K dispatch, and TILE=8 merely quadruples iterations for the same bytes (measured:
    # 13.7x worse). BLOCK_M is the only axis that changes AI.
    #
    # WHERE WE ARE. A 32K head-512 prefill dispatch measures 49.3 ms at AI=16, i.e. 178 TFLOP/s =
    # 13.7% of MI300X's 1307 TF/s bf16 peak. Not FLOP-bound (floor 6.7 ms), not HBM-bound (>99.8%
    # cache hit -- the 134 MB KV working set fits the 256 MB Infinity Cache), not DVFS (MFMA duty is
    # 13.7%). It is on-chip data movement at AI=16.
    #
    # WHAT THE H100 DOES. FlashAttention-3 does not support head_dim 512 at all (HEAD_DIMENSIONS
    # caps at 256). vLLM's fork raises SM90 to 512 and selects FwdConfig(64, 64, ...) -- tile_m 64,
    # one MMA warpgroup, ping-pong OFF. So its whole advantage here is tile_m 64 vs our BLOCK_M 16:
    # AI 64 vs 16, exactly 4x. Not TMA, not wgmma. MI300X has 1.32x MORE bf16 peak than H100.
    #
    # REGISTERS. Today's dispatches are VGPR 128 + AGPR 232 = 360 of 512. Raising BLOCK_M grows the
    # [BLOCK_M, 512] fp32 accumulator, but going 4 -> 8 warps halves its per-lane cost, so BM=64 with
    # 8 warps should land near today's 360. BM=128 is expected to spill; it is in the sweep to find
    # the wall rather than to ship.
    #
    # WHY PRIOR NEGATIVES DO NOT APPLY. NX_ATTN_WARPS_LARGE=8 was measured at BLOCK_M=16 -- 512 lanes
    # for a 16-row S-tile is degenerate and spilled (scratch_size 72). 8 warps only makes sense WITH
    # the larger BLOCK_M. The attn_head256 sweep bundled BLOCK_M=32 with 8 warps on the SLIDING
    # layers and never touched head 512; its b16w8 arm showed 8 warps alone is neutral.
    #
    # GATED to prefill-shaped work (max_seqlen_q >= 1024) so it cannot reach the MTP decode path
    # (q_len = 4) or the mixed prefill+decode batches that cost aa_c1 1.7% in the ungated TILE arm.
    # NX_PF_BM_512_MINSEQ: the BLOCK_M gate keys on CONTEXT length, not just query length.
    #
    # WHY THIS EXISTS. The first gate was `max_seqlen_q >= 1024` alone. That admitted ShareGPT:
    # vLLM's sampler caps prompts at `prompt_len > max_prompt_len(1024)` -- i.e. exactly 1024 still
    # passes -- and the harness serves ShareGPT in CHAT mode, where templating adds tokens on top of
    # the length the filter measured (datasets.py notes the recorded prompt_len is no longer the
    # served length). So long ShareGPT prompts did reach BLOCK_M=32 on the 5 full-attention layers,
    # in a short-KV batched regime it was never tuned for, and sharegpt_c128 measured -4.9%.
    #
    # ShareGPT is bounded by `prompt_len + output_len > max_total_len(2048)`, so a 4096-token
    # CONTEXT threshold excludes every ShareGPT cell with 2x margin, while long8k (8K) and long32k
    # (32K) both stay in -- they measured +8.7% and +20.6%.
    if (
        _NX_PF_BM_512
        and head_size >= 512
        and max_seqlen_q >= 1024
        and max_seqlen_k >= _NX_PF_BM_512_MINSEQ
    ):
        BLOCK_M = _NX_PF_BM_512
        BLOCK_Q = max(1, BLOCK_M // num_queries_per_kv)
        if _NX_PF_WARPS_512:
            launch_num_warps = _NX_PF_WARPS_512
            launch_num_stages = _NX_PF_STAGES_512

    # NX_ATTN_BM256: BLOCK_M for the 25 SLIDING layers (head_size 256, window 1024).
    #
    # WHY THIS IS REACHABLE WHERE head-512 IS NOT. LDS caps BLOCK_M at head 512 because the Q tile is
    # BLOCK_M * HEAD_SIZE * 2 B: at BM=64 that is 65,536 B, the entire 64 KB CU, and the launch dies
    # with OutOfResources (Required 69632). At head 256 the same tile is half as wide --
    #     BM=64 -> 32,768 B      BM=128 -> 65,536 B
    # so values impossible on the 5 full-attention layers are available on the 25 sliding ones.
    #
    # GATED ON CONTEXT, not query length, for the reason the head-512 gate is: keying only on
    # max_seqlen_q admitted chat-templated ShareGPT and 5-shot GSM8K (long query, short context) into
    # a path tuned for long context, which cost sharegpt_c128 -4.9%. ShareGPT is bounded at
    # prompt+output <= 2048, so 4096 excludes it with 2x margin.
    #
    # max_seqlen_q > 8 additionally keeps the MTP decode path (q_len = 4) out: at q_len 4 a larger
    # BLOCK_Q would only add masked query rows. Condition ported from
    # attn_bm256/code/triton_unified_attention.py.
    if (
        _NX_ATTN_BM256
        and not tuned_large_head
        and head_size == 256
        and max_seqlen_q > 8
        and max_seqlen_k >= _NX_ATTN_BM256_MINSEQ
        and num_queries_per_kv <= 16
    ):
        BLOCK_M = _NX_ATTN_BM256
        BLOCK_Q = max(1, BLOCK_M // num_queries_per_kv)
        if _NX_ATTN_WARPS_256:
            launch_num_warps = _NX_ATTN_WARPS_256
            launch_num_stages = _NX_ATTN_STAGES_256

    # NX: warp-count override for the LARGE-head path (Gemma-4's 5 full-attention layers, head_size 512).
    # Measured from our own traces: that variant uses 128 VGPR + 232 AGPR = 360 registers, which on gfx942's unified
    # 512-register file allows floor(512/360) = 1 wave/SIMD = 12.5% occupancy -- and it owns ~80% of attention time at
    # long context. The accumulator `tl.zeros([BLOCK_M, HEAD_SIZE_PADDED], float32)` is the driver: at 4 warps it is
    # 32 VGPR/lane, at 8 warps the same tile spreads over 512 lanes and costs 16. The published gfx942
    # bandwidth-vs-occupancy curve puts 1 -> 2 waves/SIMD at ~1.78x on achieved bandwidth.
    # Upstream already found 8 warps ~2x for head 256 and then gated it to Blackwell (above); nothing tunes gfx942.
    _nx_w = _nx_os.environ.get("NX_ATTN_WARPS_LARGE", "")
    if _nx_w and head_size >= 512:
        launch_num_warps = int(_nx_w)
        launch_num_stages = int(_nx_os.environ.get("NX_ATTN_STAGES_LARGE", "2"))
    ...
    # NX_PF_TILE_512: KV tile size for the head-512 layers on the PREFILL path.
    #
    # WHY. A 32K prefill census of the shipping config puts 46.5% of all prefill kernel time in
    # kernel_unified_attention, and 80.4% of THAT in the 5 full-attention layers (head 512, 2 KV
    # heads): 4,490 ms of 5,585 ms, at 94.8 ms per dispatch. They dominate because they are O(n^2)
    # over 32,768 tokens while the 25 sliding layers are O(n*1024).
    #
    # The trace shows those dispatches at VGPR 128 + AGPR 232 = 360 registers with LDS = 0. gfx942 has
    # a 512-register unified file, so 360 allows floor(512/360) = 1 wave/SIMD -- 12.5% occupancy. The
    # AGPRs hold the K and V tiles: at TILE=32 and head 512 each tile is 32*512*2 B = 32 KB, K and V
    # together 64 KB per workgroup. Halving the tile should roughly halve that and may buy a second
    # wave; the cost is more loop iterations over the same bytes.
    #
    # PREFILL-ONLY, so there is NO acceptance risk: speculative verification runs on the 3-D decode
    # path, which reads TILE_SIZE_DECODE. An earlier NX_FULLATTN_TILE arm was rejected on an
    # acceptance move, but that was measured on DECODE; this knob cannot reach that path.
    # Unset => 0 => stock behaviour.
    # SEQUENCE-LENGTH GATED. The ungated sweep measured TILE=16 at +3.0% on BOTH long cells (32K and
    # 8K) but -1.7% on aa_c1 -- outside that cell's 0.36% floor. aa_c1 is a ~10K prefill, where the
    # O(n^2) attention term has not yet grown enough to pay for the extra loop iterations a smaller
    # tile costs. So apply the smaller tile only once the sequence is long enough for the quadratic
    # term to dominate. NX_PF_TILE_512_MINSEQ is that threshold (0 = always, i.e. the old behaviour).
    #
    # TILE=8 was catastrophic (TTFT 13.7x worse) -- halving again doubles iterations while the
    # per-iteration overhead stays fixed, so the kernel becomes pure overhead. This axis has a sharp
    # floor, not a smooth gradient; do not go below 16.
    # max_seqlen_q >= 1024 IS LOAD-BEARING, for two separately verified reasons.
    #
    # 1. TILE_SIZE_PREFILL is a misnomer. The launch picks `tile_size = TILE_SIZE_PREFILL` for the
    #    whole 2D path -- which includes DECODE; only the 3D segmented path reads TILE_SIZE_DECODE.
    #    So changing it changes decode too.
    # 2. max_seqlen_k is NOT the real context length. A gate census over one sharegpt_c128 run
    #    (gatedbg/results/gate_census.csv) recorded max_seqlen_k == 131072 -- the full
    #    max-model-len -- on 5,192 of 26,578 calls (19.5%), against a true ShareGPT context of
    #    <= 2048. Those are MTP decode calls (q_len = 4).
    #
    # Together those made the context-only gate fire 1,028 times on sharegpt_c128 and run its DECODE
    # at TILE=16 instead of 32, costing a real, reproducible -0.81% (five measurements per arm,
    # non-overlapping, and surviving an order-reversal test). Requiring prefill-shaped work fixes it:
    # the two BLOCK_M gates already carry this condition and the same census shows they fired ZERO
    # times on that cell.
    if (
        _NX_PF_TILE_512
        and head_size >= 512
        and max_seqlen_q >= 1024
        and max_seqlen_k >= _NX_PF_TILE_512_MINSEQ
    ):
        TILE_SIZE_PREFILL = _NX_PF_TILE_512
    ...
    # NX (PR #56148): multi-query split-K needs proven-uniform decode metadata and a bounded causal verify shape.
    # Without NX_VERIFY_SPLIT this reduces exactly to the shipped `max_seqlen_q > 1` rejection.
    use_3d = not (
    ...
    if use_3d:
        # The scratch is indexed by QUERY TOKEN, not by sequence, so a verify batch needs num_seqs*query_len rows.
        # Both kernels use packed FP32 offsets with no stride arguments, so shape/dtype/contiguity must match exactly;
        # a builder that did not resize its buffers falls back to 2D here instead of mis-indexing.
