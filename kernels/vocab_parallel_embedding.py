...
# BH6 (autonomous_exps/BH6_fp8_lm_head): ParallelLMHead through FP8 PTPC scaled-mm. Unset/0 -> unchanged.
_BH6_FP8_LM_HEAD = os.environ.get("BH6_FP8_LM_HEAD", "0") == "1"


class Fp8PtpcLMHeadMethod(UnquantizedEmbeddingMethod):
    """BH6: FP8 PTPC logits projection, composed from Fp8PtpcOnlineLinearMethod
    (quantization/online/fp8.py:339-422). The BF16 `weight` is kept for embedding
    lookups / tied heads; the FP8 copy is `layer.bh6_fp8`."""
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if isinstance(self.fp8_linear, MarlinFP8ScaledMMLinearKernel):
            raise ValueError(
                "BH6: FP8 PTPC LM head needs a per-token activation FP8 kernel; "
                "MarlinFP8 is W8A16 weight-only."
            )

    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        super().process_weights_after_loading(layer)
        if getattr(layer, "bh6_fp8", None) is not None:
            return
        ...
        # fp8.py:390-401 (per-output-row scale; the head is only output-dim sharded)
        ...
        # fp8.py:403
        self.fp8_linear.process_weights_after_loading(fp8)
        layer.bh6_fp8 = fp8
        init_logger(__name__).info(
            "BH6: FP8 PTPC LM head %s x %s (%s)",
            qweight.shape[0],
            qweight.shape[1],
    ...
    def apply(
        ...
        fp8 = getattr(layer, "bh6_fp8", None)
        if fp8 is None:  # not yet processed (no weights loaded): BF16 path
            return super().apply(layer, x, bias)
        # fp8.py:422
        return self.fp8_linear.apply_weights(fp8, x, bias)
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            and _BH6_FP8_LM_HEAD
            and isinstance(self, ParallelLMHead)
        ):
            quant_method = Fp8PtpcLMHeadMethod()
