...
import os as _nx_os
_NX_FUSED_ACTQUANT = _nx_os.environ.get("NX_FUSED_ACTQUANT", "0") == "1"
...
class TritonExperts(LoRAExpertsMixin, mk.FusedMoEExpertsModular):
    ...
    def apply(
        ...
        if (
            ...
            # NX: fuse GELU-tanh+mul with the per-token FP8 quantisation.
            # The branch above already does exactly this for SILU + per-BLOCK quant via the C++ op
            # ops.silu_and_mul_per_block_quant. Gemma-4 is GELU_TANH + per-TOKEN, so it fell to the
            # unfused path below: two kernels per layer, 30 + 30 dispatches/step at 2.50 and 2.68 us,
            # 92% and 86% of which is this box's measured 2.31 us per-dispatch floor. Fusing removes
            # 30 dispatches ~= 69 us of a 4604 us step ~= 1.5%.
            # Extending the C++ op would mean rebuilding vLLM; a Triton kernel is JIT-compiled.
            # Gated narrowly (per-token, no block shape, no LoRA, no emulation) so every other caller
            # keeps the reference path, and NX_FUSED_ACTQUANT=0 restores it unconditionally.
            from vllm.nx_fused_act_quant import gelu_tanh_and_mul_per_token_quant
            qintermediate_cache2, a2q_scale = gelu_tanh_and_mul_per_token_quant(
                intermediate_cache1.view(-1, N), self.quant_dtype
            )
        else:
            self.activation(
